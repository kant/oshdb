...
public class ConsoleHandler extends LoaderHandler {
  ...
  public void handleWayGrid(
    ...
    final long id = ZGrid.getIdWithoutZoom(cellId);
    System.out.printf(
        "load way grid (%d:%d %d) ways:%d nodes:%d%n", zoom, id, cellId, ways.size(), nodes.size());
  }
  ...
  public void handleRelationGrid(
    ...
    final long id = ZGrid.getIdWithoutZoom(cellId);
    System.out.printf(
        "load relation grid (%d:%d %d) ways:%d nodes:%d ways:%d%n",
        zoom, id, cellId, entities.size(), nodes.size(), ways.size());
  }
}
