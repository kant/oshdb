...
public abstract class OSHDBHandler extends LoaderHandler {
  ...
  protected OSHDBHandler(
      Roaring64NavigableMap bitmapNodeRelation, Roaring64NavigableMap bitmapWayRelation) {
  ...
  public void handleNodeGrid(long zId, Collection<TransformOSHNode> nodes) {
    ...
    try {
      if (gridNodes.size() != 0) {
        ...
        GridOSHNodes grid =
            GridOSHNodes.rebase(
                xyId, zoom, gridNodes.get(0).getId(), 0, longitude, latitude, gridNodes);
        handleNodeGrid(grid);
      } else {
  ...
  @Override
  public void handleWayGrid(
      long zId, Collection<TransformOSHWay> ways, Collection<TransformOSHNode> nodes) {
    if (zId < 0) return;
    int zoom = ZGrid.getZoom(zId);
    XYGrid xyGrid = new XYGrid(zoom);
    ...
    try {
      if (gridWays.size() != 0) {
        GridOSHWays grid =
            GridOSHWays.compact(
                xyId, zoom, gridWays.get(0).getId(), 0, longitude, latitude, gridWays);
        handleWayGrid(grid);
      }
    } catch (IOException e) {
      throw new RuntimeException(e);
  ...
  @Override
  public void handleRelationGrid(
      long zId,
      Collection<TransfomRelation> entities,
            ...
            .map(
                osh2 -> {
                  ...
                  for (long id : osh2.getNodeIds()) {
                    ...
                    if (node != null) nodesForThisRelation.add(node);
                  }
                  ...
                  for (long id : osh2.getWayIds()) {
                    ...
                    if (way != null) waysForThisRelation.add(way);
                  }

                  try {

                    OSHRelation ret =
                        OSHRelation.build(versions, nodesForThisRelation, waysForThisRelation);
    ...
    try {
      GridOSHRelations grid =
          GridOSHRelations.compact(
              xyId, zoom, gridRelation.get(0).getId(), 0, longitude, latitude, gridRelation);
