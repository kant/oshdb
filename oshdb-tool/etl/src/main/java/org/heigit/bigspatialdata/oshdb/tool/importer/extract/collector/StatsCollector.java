...
public class StatsCollector {
  ...
  public void print(PrintStream out) {
    ...
    if (header != null) {
      ...
      out.printf(
          "header.bbox=%d,%d,%d,%d%n",
          bbox.getLeft(), bbox.getBottom(), bbox.getRight(), bbox.getTop());
    ...
    if (minLon != Integer.MAX_VALUE) {
      out.printf(
          "data.bbox=%.7f,%.7f,%.7f,%.7f%n",
          minLon * OSHDB.GEOM_PRECISION,
          minLat * OSHDB.GEOM_PRECISION,
          maxLon * OSHDB.GEOM_PRECISION,
          maxLat * OSHDB.GEOM_PRECISION);
    }

    out.printf(
        "data.timerange=%s,%s%n",
    ...
    out.println(
        "data.nodes="
            + Arrays.stream(nodes, 1, nodes.length)
                .collect(
                    () -> new StringBuilder("" + nodes[0]),
                    (sb, i) -> sb.append(",").append(i),
                    (a, b) -> a.append(b))
                .toString());
    out.println(
        "data.ways="
            + Arrays.stream(ways, 1, ways.length)
                .collect(
                    () -> new StringBuilder("" + ways[0]),
                    (sb, i) -> sb.append(",").append(i),
                    (a, b) -> a.append(b))
                .toString());
    out.println(
        "data.relations="
            + Arrays.stream(relations, 1, relations.length)
                .collect(
                    () -> new StringBuilder("" + relations[0]),
                    (sb, i) -> sb.append(",").append(i),
                    (a, b) -> a.append(b))
                .toString());
    if (unknown[0] > 0)
      out.println(
          "data.unknown="
              + Arrays.stream(unknown, 1, unknown.length)
                  .collect(
                      () -> new StringBuilder("" + unknown[0]),
                      (sb, i) -> sb.append(",").append(i),
                      (a, b) -> a.append(b))
                  .toString());
  }
