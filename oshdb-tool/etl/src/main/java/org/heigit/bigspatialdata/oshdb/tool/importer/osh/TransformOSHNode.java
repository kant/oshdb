...
public class TransformOSHNode extends OSHNode2 {
  ...
  public static TransformOSHNode build(
      ByteArrayOutputWrapper output,
      ByteArrayOutputWrapper record,
      ByteArrayOutputWrapper aux,
      List<OSMNode> versions,
      final long baseId,
    ...
    return TransformOSHNode.instance(
        record.array(), 0, record.length(), baseId, baseTimestamp, baseLongitude, baseLatitude);
  }

  public static TransformOSHNode instance(
      final byte[] data,
      final int offset,
      final int length,
      final long baseId,
  ...
  public static TransformOSHNode instance(final byte[] data, final int offset, final int length)
      throws IOException {
    return instance(data, offset, length, 0, 0, 0, 0);
  }

  private TransformOSHNode(
      final byte[] data,
      final int offset,
      final int length,
      final byte header,
