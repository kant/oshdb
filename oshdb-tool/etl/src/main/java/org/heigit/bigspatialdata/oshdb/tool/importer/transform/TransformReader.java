...
public class TransformReader {
  ...
  public static class NodeReaders {
    PriorityQueue<NodeReader> queue =
        new PriorityQueue<>(
            (a, b) -> {
  ...
  public static void load(
      List<Grid<TransformOSHWay>> zoomLevel, int zoom, Long2ObjectMap<OSHNode> nodes) {
  ...
  */
  ...
  private static Set<Long> getUpperLevelRefMapSet(List<Grid<TransformOSHWay>> zoomLevel, int zoom) {
    while (zoom > 0) {
      Grid<TransformOSHWay> grid = zoomLevel.get(--zoom);
      if (grid != null) return grid.refMapSet;
    }
    return null;
  }
  ...
  public static void main(String[] args) throws FileNotFoundException, IOException {
    ...
    WayReader wayReader =
        new WayReader(
            workDirectory.resolve(
                String.format("transform_%s_%02d", OSMType.WAY.toString().toLowerCase(), 0)));
    ...
    NodeReader nodeReader =
        new NodeReader(
            workDirectory.resolve(
                String.format("transform_%s_%02d", OSMType.NODE.toString().toLowerCase(), 0)));
    ...
    while (true) {
      ...
      ways.stream()
          ...
          .forEach(
              ids -> {
                for (long id : ids) nodeRefMapSet.add(id);
              });
      ...
      if (upperRefMapSet != null) nodeRefMapSet.addAll(upperRefMapSet);
      ...
      while (nodeReader.cellId <= cellId) {
        for (OSHNode node : nodeReader.get()) {
          if (upperRefMapSet.contains(node.getId())) nodes.put(node.getId(), node);
        }
    ...
    */
    // System.out.printf("->node:%d=%d%n",ZGrid.getZoom(reader.get(0).cellId),
    // ZGrid.getIdWithoutZoom(reader.get(0).cellId));
