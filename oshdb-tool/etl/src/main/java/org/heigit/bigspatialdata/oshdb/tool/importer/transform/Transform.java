...
public class Transform {
  ...
  public void transformNodes(
      OsmPbfMeta pbfMeta, int maxZoom, TagToIdMapper tag2Id, int workerId, int workerTotal)
      throws IOException {
    final Transformer transformer =
        new TransformerNode(maxMemory, maxZoom, workDirectory, tag2Id, workerId);
    Flowable<List<Entity>> flow =
        RxOshPbfReader //
  ...
  public void transformWays(
      OsmPbfMeta pbfMeta,
      int maxZoom,
      TagToIdMapper tag2Id,
      SortedLong2LongMap node2cell,
      int workerId,
      int workerTotal)
      throws IOException {
    final Transformer transformer =
  ...
  public void transformRelations(
      ...
      int maxZoom,
      TagToIdMapper tag2Id,
      RoleToIdMapper role2Id,
      SortedLong2LongMap node2cell,
      SortedLong2LongMap way2cell,
      int workerId,
      int workerTotal)
      throws IOException {
    final Transformer transformer =
        new TransformerRelation(
            maxMemory, maxZoom, workDirectory, tag2Id, role2Id, node2cell, way2cell, workerId);
    Flowable<List<Entity>> flow =
        RxOshPbfReader //
            .readOsh(
                pbfMeta.pbf, pbfMeta.relationStart, pbfMeta.relationEnd, pbfMeta.relationEnd) //
            .map(osh -> osh.getVersions());
    subscribe(flow, transformer::transform, transformer::error, transformer::complete);
  }
  ...
  private static <T> void subscribe(
    ...
    FlowableBlockingSubscribe.subscribe(
        o, new MyLambdaSubscriber<T>(onNext, onError, onComplete, 1L));
  }
  ...
  public static void transform(TransformArgs config) throws Exception {
    ...
    final long availableHeapMemory =
        SizeEstimator.estimateAvailableMemory(); // reserve 1GB for parsing
    final long availableMemory =
        availableHeapMemory
            - Math.max(
                1 * GB,
                availableHeapMemory / 3); // reserve at least 1GB or 1/3 of the total memory
    ...
    if (step.startsWith("a") || step.startsWith("n")) {
      ...
      Transform.withMaxMemory(maxMemory)
          .withWorkDirectory(workDir)
          .transformNodes(pbfMeta, maxZoom, tag2Id, worker, workerTotal);
      System.out.println(" done!");
    }

    if (step.startsWith("a") || step.startsWith("w")) {
      final long mapMemory = availableMemory / 2L;
      final SortedLong2LongMap node2Cell =
      ...
      Transform.withMaxMemory(maxMemory)
          .withWorkDirectory(workDir)
          .transformWays(pbfMeta, maxZoom, tag2Id, node2Cell, worker, workerTotal);
      System.out.println(" done!");
    }
    ...
    if (step.startsWith("a") || step.startsWith("r")) {
      ...
      final SortedLong2LongMap node2Cell =
          new SortedLong2LongMap(workDir.resolve("transform_idToCell_" + "node"), mapMemoryNode);
      final SortedLong2LongMap way2Cell =
          new SortedLong2LongMap(
      ...
      Transform.withMaxMemory(maxMemory)
          .withWorkDirectory(workDir)
          .transformRelations(
              pbfMeta, maxZoom, tag2Id, role2Id, node2Cell, way2Cell, worker, workerTotal);
      System.out.println(" done!");
    }
  }
