...
public class JSONTransformerTest {
  ...
  public void testTransform_4args_101() throws SQLException, OSHDBKeytablesNotFoundException {
    int[] properties = {1, 2};
    OSMEntity entity =
        new OSMNode(1L, 1, new OSHDBTimestamp(0L), 1L, 1, properties, 1000000000L, 1000000000L);
    ...
    String result =
        JSONTransformer.transform(entity, timestamp, tt, new FakeTagInterpreter()).toString();
  ...
  public void testTransform_4args_102()
    ...
    versions.add(
        new OSMNode(123l, 1, new OSHDBTimestamp(0l), 1l, USER_A, TAGS_A, LONLAT_A[0], LONLAT_A[1]));
    OSHNode hnode = OSHNode.build(versions);
    OSMMember part = new OSMMember(1L, OSMType.NODE, 0, hnode);
    OSMRelation instance =
        new OSMRelation(
            1L, 1, new OSHDBTimestamp(0L), 1L, 1, new int[] {1, 2}, new OSMMember[] {part, part});
    TagTranslator tt =
        new TagTranslator(
            DriverManager.getConnection("jdbc:h2:./src/test/resources/test-data", "sa", ""));
    String expResult =
  ...
  public void testTransform_4args_103()
    ...
    List<OSMNode> versions = new ArrayList<>(1);
    versions.add(
        new OSMNode(123l, 1, new OSHDBTimestamp(0l), 0l, USER_A, TAGS_A, LONLAT_A[0], LONLAT_A[1]));
    OSHNode hnode = OSHNode.build(versions);
    ...
    String result =
        JSONTransformer.transform(instance, new OSHDBTimestamp(1L), tt, new FakeTagInterpreter())
            .toString();
    assertEquals(expResult, result);
  }
  ...
  public void testMultiTransform() throws SQLException, OSHDBKeytablesNotFoundException {
    int[] properties = {1, 2};
    OSMNode instance =
        new OSMNode(1L, 1, new OSHDBTimestamp(1L), 1L, 1, properties, 1000000000L, 1000000000L);
  ...
  public void testTransform_4args_2()
    ...
    versions.add(
        new OSMNode(123l, 2, new OSHDBTimestamp(0l), 46l, 1, TAGS_A, LONLAT_A[0], LONLAT_A[1]));
    versions.add(
        new OSMNode(123l, 1, new OSHDBTimestamp(1l), 47l, 2, TAGS_B, LONLAT_B[0], LONLAT_B[1]));
    ...
    String result = JSONTransformer.transform(instance, tt, new FakeTagInterpreter()).toString();
    assertEquals(expResult, result);
  }
