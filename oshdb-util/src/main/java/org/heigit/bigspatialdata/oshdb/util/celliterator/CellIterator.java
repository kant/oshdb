...
public class CellIterator implements Serializable {
   ...
   *     multipolygons".
   *     <p>Note, that if includeOldStyleMultipolygons is true, for each old style multipolygon only
   *     the geometry of the inner holes are returned (while the outer part is already present as
   *     the respective way's output)! This has to be interpreted separately and differently in the
   *     data analysis! The includeOldStyleMultipolygons is also quite a bit less efficient (both
   *     CPU and memory) as the default path.
   */
  ...
  public CellIterator(
    ...
    this.isBoundByPolygon =
        false; // todo: is this flag even needed? -> replace by "dummy" polygonClipper?
  ...
  public static class IterateByTimestampEntry {
    ...
    public IterateByTimestampEntry(
        ...
        LazyEvaluatedObject<Geometry> unclippedGeom) {
   ...
   *     timestamp. If an object has not been modified between timestamps, the output may contain
   *     the *same* Geometry object in the output multiple times. This can be used to optimize away
   *     recalculating expensive geometry operations on unchanged feature geometries later on in the
   *     code.
   */
  public Stream<IterateByTimestampEntry> iterateByTimestamps(GridOSHEntity cell) {
    ...
    if (isBoundByPolygon) {
      ...
      OSHDBBoundingBox cellBoundingBox =
          XYGrid.getBoundingBox(new CellId(cell.getLevel(), cell.getId()));
    ...
    return StreamSupport.stream(cellData.spliterator(), false)
        .flatMap(
            oshEntity -> {
              ...
              // optimize loop by requesting modification timestamps first, and skip geometry
              // calculations
              // where not needed
              ...
              for (Map.Entry<OSHDBTimestamp, OSMEntity> entity : osmEntityByTimestamps.entrySet()) {
                  ...
                  for (int i = 0; i < rel.getMembers().length; i++) {
                    ...
                    if (relMember.getType() == OSMType.WAY
                      ...
                      if (!osmEntityFilter.test(way)) {
                        ...
                        // we know this multipolygon only has exactly one outer way, so we can abort
                        // the
                        // loop and actually
                ...
                try {
                  ...
                  if (!isOldStyleMultipolygon) {
                    ...
                    // old style multipolygons: return only the inner holes of the geometry -> this
                    // is then
                    // used to "fix" the
                    // results obtained from calculating the geometry on the object's outer way
                    // which
                            ...
                            () -> {
                              Geometry _geom =
                                  OSHDBGeometryBuilder.getGeometry(
                                      osmEntity, timestamp, tagInterpreter);
                              ...
                              for (int i = 0; i < poly.getNumInteriorRing(); i++) {
                                interiorRings[i] =
                                    new Polygon(
                                        (LinearRing) poly.getInteriorRingN(i),
                                        new LinearRing[] {},
                                        gf);
                              }
                  ...
                  if (fullyInside || !geom.get().isEmpty()) {
                    LazyEvaluatedObject<Geometry> fullGeom =
                        fullyInside
                            ? geom
                            : new LazyEvaluatedObject<>(
                                () ->
                    ...
                    // add skipped timestamps (where nothing has changed from the last timestamp) to
                    // result
                  ...
                  // maybe some corner case where JTS doesn't support operations on a broken
                  // geometry
                  LOG.info(
                      "Entity {}/{} skipped because of invalid geometry at timestamp {}",
                      osmEntity.getType().toString().toLowerCase(),
                      osmEntity.getId(),
                      timestamp);
                } catch (TopologyException err) {
                  // happens e.g. in JTS intersection method when geometries are self-overlapping
                  LOG.info(
                      "Topology error with entity {}/{} at timestamp {}: {}",
                      osmEntity.getType().toString().toLowerCase(),
                      osmEntity.getId(),
                      timestamp,
                      err.toString());
  ...
  private LazyEvaluatedObject<Geometry> constructClippedGeometry(
      OSMEntity osmEntity, OSHDBTimestamp timestamp, boolean fullyInside) {
    LazyEvaluatedObject<Geometry> geom;
    if (fullyInside) {
      geom =
          new LazyEvaluatedObject<>(
      ...
      geom =
          new LazyEvaluatedObject<>(
              fastPolygonClipper.intersection(
                  OSHDBGeometryBuilder.getGeometry(osmEntity, timestamp, tagInterpreter)));
    } else {
      geom =
          new LazyEvaluatedObject<>(
  ...
  public static class IterateAllEntry {
    public final OSHDBTimestamp timestamp;
    @Nonnull public final OSMEntity osmEntity;
    ...
    return StreamSupport.stream(cellData.spliterator(), false)
        .flatMap(
            oshEntity -> {
              ...
              if (modTs.size() == 0
                  || !timeInterval.intersects(
                      new OSHDBTimestampInterval(modTs.get(0), modTs.get(modTs.size() - 1)))) {
                // ignore osh entity because it's edit history is fully outside of the given time
                // interval
                // of interest
                return Stream.empty();
              }
              ...
              for (Map.Entry<OSHDBTimestamp, OSMEntity> entity : osmEntityByTimestamps.entrySet()) {
                ...
                // todo: replace with variable outside of osmEntitiyLoop (than we can also get rid
                // of
                ...
                if (!timeInterval.includes(timestamp)) {
                  ...
                  if (timeInterval.compareTo(timestamp)
                      > 0) { // timestamp in the future of the interval
                    break; // abort current osmEntityByTimestamps loop, continue with next osh
                           // entity
                  } else if (!timeInterval.includes(
                      nextTs)) { // next modification state is also in not in
                    ...
                    // skip it
                    // but we still have to process this entity fully, because we need stuff in
                    // `prev` for
                  ...
                  if (prev != null && !prev.activities.contains(ContributionType.DELETION)) {
                    prev =
                        new IterateAllEntry(
                            ...
                            prev.osmEntity,
                            oshEntity,
                            new LazyEvaluatedObject<>((Geometry) null),
                            prev.geometry,
                            new LazyEvaluatedObject<>((Geometry) null),
                            prev.unclippedGeometry,
                            new LazyEvaluatedContributionTypes(
                                EnumSet.of(ContributionType.DELETION)),
                            osmEntity.getChangeset());
                    // cannot normally happen, because prev is never null while skipOutput is true
                    // (since no
                    // previous result has yet been generated before the first modification in the
                    // query
                    // timestamp inteval). But if the oshdb-api would at some point have to support
                    // non-
                ...
                if (includeOldStyleMultipolygons
                  ...
                  if (!osmEntityFilter.test(osmEntity)) {
                    ...
                    // TODO?: separate/additional activity type (e.g. "RECYCLED" ??) and still
                    // construct
                ...
                try {
                  ...
                  if (!isOldStyleMultipolygon) {
                    ...
                    // old style multipolygons: return only the inner holes of the geometry -> this
                    // is then
                    // used to "fix" the results obtained from calculating the geometry on the
                    // object's outer
                  ...
                  if (!fullyInside && geom.get().isEmpty()) {
                    ...
                    // todo: special case when an object gets specific tag/condition again after
                    // having them
                    // removed?
                  } else {
                            ...
                            contributionType -> {
                              switch (contributionType) {
                                case TAG_CHANGE:
                                  ...
                                  if (prevEntity.getRawTags().length
                                      != osmEntity.getRawTags().length) {
                                    tagsChange = true;
                                  } else {
                                ...
                                case GEOMETRY_CHANGE:
                                  ...
                                  if (geom.get() != null && prevGeometry.get() != null) {
                                    // todo: what if both are null? -> maybe fall back to
                                    // MEMBER_CHANGE?
