...
class BaseTagInterpreter implements TagInterpreter {
  ...
  private boolean evaluateWayForArea(OSMWay entity) {
    ...
    if (entity.hasTagValue(areaNoTagKeyId, areaNoTagValueId)) return false;
    for (int i = 0; i < tags.length; i += 2) {
      if (wayAreaTags.containsKey(tags[i]) && wayAreaTags.get(tags[i]).contains(tags[i + 1]))
        return true;
    }
    return false;
  }
  ...
  private boolean evaluateRelationForArea(OSMRelation entity) {
    ...
    for (int i = 0; i < tags.length; i += 2) {
      if (relationAreaTags.containsKey(tags[i])
          && relationAreaTags.get(tags[i]).contains(tags[i + 1])) return true;
    }
    return false;
  }
  ...
  public boolean hasInterestingTagKey(OSMEntity osm) {
    ...
    for (int i = 0; i < tags.length; i += 2) {
      if (!uninterestingTagKeys.contains(tags[i])) return true;
    }
    return false;
  }
  ...
  public boolean isOldStyleMultipolygon(OSMRelation osmRelation) {
    ...
    for (int i = 0; i < tags.length; i += 2) {
      if (relationAreaTags.containsKey(tags[i])
          && relationAreaTags.get(tags[i]).contains(tags[i + 1])) continue;
      if (!uninterestingTagKeys.contains(tags[i])) return false;
    }
    return true;
  }
  ...
  public boolean isMultipolygonOuterMember(OSMMember osmMember) {
    ...
    return roleId == this.outerRoleId
        || roleId
            == this
                .emptyRoleId; // some historic osm data may still be mapped without roles set ->
                              // assume empty roles to mean outer
    // todo: check if there is need for some more clever outer/inner detection for the empty role
    // case with old data
  }
