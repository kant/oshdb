 ...
 */
public class FastBboxOutsidePolygon extends FastInPolygon
    implements Predicate<OSHDBBoundingBox>, Serializable {
  ...
  public <P extends Geometry & Polygonal> FastBboxOutsidePolygon(P geom) {
    ...
    if (geom instanceof Polygon) {
      ...
      for (int i = 0; i < mp.getNumGeometries(); i++) polys.add((Polygon) mp.getGeometryN(i));
    }
  ...
  public boolean test(OSHDBBoundingBox boundingBox) {
    ...
    for (Envelope innerBBox : outerBboxes) {
      if (boundingBox.getMinLat() <= innerBBox.getMinY()
        ...
        return false; // the bounding box fully covers at least one of the (multi)polygon's outer
                      // rings
      }
    }
    return true;
