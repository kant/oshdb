 ...
 *
 * <p>Depending on whether a plain MapReducer or a MapAggregator object the result will either be a
...
interface MapReducerAggregations<X> {
   ...
   *
   * <p>This can be used to perform an arbitrary reduce routine
   *
   ...
   *
   * <p>Functionally, this interface is similar to Java8 Stream's <a
   * href="https://docs.oracle.com/javase/8/docs/api/java/util/stream/Stream.html#reduce-T-java.util.function.BinaryOperator-">reduce(identity,accumulator)</a>
   * interface.
   *
   * @param identitySupplier a factory function that returns a new starting value to reduce results
   *     into (e.g. when summing values, one needs to start at zero)
   ...
   *     and `combiner` steps)
   */
  Object reduce(SerializableSupplier<X> identitySupplier, SerializableBinaryOperator<X> accumulator)
      throws Exception;
