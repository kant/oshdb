...
public class TestCollect {
  ...
  public void testCollect() throws Exception {
    List<OSMContribution> result =
        this.createMapReducerOSMContribution().timestamps(timestamps72).collect();
    assertEquals(
        42,
        result
            .stream()
  ...
  public void testFlatMapCollect() throws Exception {
    List<Long> result =
        this.createMapReducerOSMContribution()
            .timestamps(timestamps72)
            .flatMap(
                contribution -> Collections.singletonList(contribution.getEntityAfter().getId()))
            .collect();
    assertEquals(42, result.stream().collect(Collectors.toSet()).size());
  }
  ...
  public void testFlatMapCollectGroupedById() throws Exception {
    List<Long> result =
        this.createMapReducerOSMContribution()
            ...
            .flatMap(
                contributions ->
                    Collections.singletonList(contributions.get(0).getEntityAfter().getId()))
            .collect();
    assertEquals(42, result.stream().collect(Collectors.toSet()).size());
  }
  ...
  public void testAggregatedByTimestamp() throws Exception {
    SortedMap<OSHDBTimestamp, List<Long>> result =
        this.createMapReducerOSMContribution()
            .timestamps(timestamps72)
