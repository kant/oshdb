...
public class TestHelpersOSMContributionView {
  ...
  public void testSum() throws Exception {
    // single timestamp
    SortedMap<OSHDBTimestamp, Number> result1 =
        this.createMapReducer()
            .timestamps(timestamps2)
            .aggregateByTimestamp()
            .sum(
                contribution ->
                    contribution.getContributionTypes().contains(ContributionType.TAG_CHANGE)
                        ? 1
                        : 0);
    ...
    // many timestamps
    SortedMap<OSHDBTimestamp, Number> result2 =
        this.createMapReducer()
    ...
    assertEquals(
        42, result2.values().stream().reduce(0, (acc, num) -> acc.intValue() + num.intValue()));
    ...
    SortedMap<String, Number> result4 =
        this.createMapReducer()
            .timestamps(timestamps72)
            .osmType(OSMType.WAY)
            .osmTag("building", "yes")
  ...
  public void testCount() throws Exception {
    ...
    SortedMap<OSHDBTimestamp, Integer> result1 =
        this.createMapReducer().timestamps(timestamps2).aggregateByTimestamp().count();
    ...
    SortedMap<OSHDBTimestamp, Integer> result2 =
        this.createMapReducer().timestamps(timestamps72).aggregateByTimestamp().count();
    ...
    // total
    Integer result3 = this.createMapReducer().timestamps(timestamps72).count();

    assertEquals(70, result3.intValue());

    // custom aggregation identifier
    SortedMap<Boolean, Integer> result4 =
        this.createMapReducer()
            .timestamps(timestamps2)
  ...
  public void testAverage() throws Exception {
    ...
    // many timestamps
    SortedMap<OSHDBTimestamp, Double> result2 =
        this.createMapReducer()
            .timestamps(timestamps72)
            .aggregateByTimestamp()
            .map(
    ...
    // custom aggregation identifier
    SortedMap<Boolean, Double> result4 =
        this.createMapReducer()
            .timestamps(timestamps72)
            .aggregateBy(
                contribution ->
  ...
  public void testUniq() throws Exception {
    // single timestamp
    SortedMap<OSHDBTimestamp, Set<Long>> result1 =
        this.createMapReducer()
            .timestamps(timestamps2)
            .aggregateByTimestamp()
    ...
    // many timestamps
    SortedMap<OSHDBTimestamp, Set<Long>> result2 =
        this.createMapReducer()
            .timestamps(timestamps72)
            .aggregateByTimestamp()
            .uniq(contribution -> contribution.getEntityAfter().getId());
    ...
    // custom aggregation identifier
    SortedMap<Boolean, Set<Long>> result4 =
        this.createMapReducer()
            .timestamps(timestamps72)
            .aggregateBy(contribution -> contribution.getEntityAfter().getId() % 2 == 0)
            .uniq(contribution -> contribution.getEntityAfter().getId());
  ...
  public void testIssue107() throws Exception {
    // single timestamp
    List<OSMContribution> result = this.createMapReducer().timestamps(timestamps72).collect();
