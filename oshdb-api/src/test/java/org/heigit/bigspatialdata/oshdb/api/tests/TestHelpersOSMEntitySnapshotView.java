...
public class TestHelpersOSMEntitySnapshotView {
  ...
  public void testSum() throws Exception {
    ...
    SortedMap<OSHDBTimestamp, Number> result1 =
        this.createMapReducer().timestamps(timestamps1).aggregateByTimestamp().sum(snapshot -> 1);
    ...
    SortedMap<Boolean, Number> result4 =
        this.createMapReducer()
            .timestamps(timestamps1)
            .osmType(OSMType.WAY)
            .osmTag("building", "yes")
    ...
    Double result1 =
        this.createMapReducer()
            ...
            .weightedAverage(
                snapshot ->
                    new WeightedValue<>(
                        snapshot.getEntity().getId() % 2, 1 * (snapshot.getEntity().getId() % 2)));
    ...
    SortedMap<OSHDBTimestamp, Double> result2 =
        this.createMapReducer()
            ...
            .weightedAverage(
                snapshot ->
                    new WeightedValue<>(
                        snapshot.getEntity().getId() % 2, 2 * (snapshot.getEntity().getId() % 2)));
