...
public class TestFlatMapAggregate {
  ...
  public void test() throws Exception {
    SortedMap<Long, Set<Pair<Integer, Integer>>> result =
        createMapReducerOSMContribution()
            .timestamps(timestamps72)
            .flatMap(
                contribution -> {
                  if (contribution.getEntityAfter().getId() != 617308093) return new ArrayList<>();
            ...
            .reduce(
                ...
                (x, y) -> {
                  Set<Pair<Integer, Integer>> ret = new HashSet<>(x);
                  ret.addAll(y);
                  return ret;
                });
