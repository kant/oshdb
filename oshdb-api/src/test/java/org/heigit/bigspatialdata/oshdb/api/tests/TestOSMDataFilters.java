...
public class TestOSMDataFilters {
  ...
  public void multiPolygon() throws Exception {
    ...
    Integer result =
        createMapReducerOSMEntitySnapshot()
            .osmType(OSMType.NODE)
            .areaOfInterest(
                gf.createMultiPolygon(new Polygon[] {OSHDBGeometryBuilder.getGeometry(bbox)}))
            .timestamps(timestamps1)
            .count();
    assertEquals(2, result.intValue());
  }
  ...
  public void tagKey() throws Exception {
    SortedMap<OSMType, Integer> result =
        createMapReducerOSMEntitySnapshot()
            .osmTag("building")
            .areaOfInterest(bbox)
            .timestamps(timestamps1)
  ...
  public void tagList() throws Exception {
    Integer result =
        createMapReducerOSMEntitySnapshot()
            .osmTag(
                Arrays.asList(
                    new OSMTag("highway", "residential"), new OSMTag("highway", "unclassified")))
            .osmType(OSMType.WAY)
            .areaOfInterest(bbox)
            .timestamps(timestamps1)
            .count();
        ...
        createMapReducerOSMEntitySnapshot()
            ...
            .uniq(
                snapshot -> {
                  ...
                  return -1; // cannot actually happen (since we query only snapshots with a name,
                             // but needed to make Java's compiler happy
                });
