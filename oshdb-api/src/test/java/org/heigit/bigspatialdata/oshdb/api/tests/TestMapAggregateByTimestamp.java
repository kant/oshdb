...
public class TestMapAggregateByTimestamp {
  ...
  public void testOSMContribution() throws Exception {
    ...
    assertEquals(
        39, result2.entrySet().stream().map(Map.Entry::getValue).reduce(-1, Math::max).intValue());
  }
  ...
  public void testOSMContributionCustomDefault() throws Exception {
    ...
    SortedMap<OSHDBTimestamp, Integer> resultAuto =
        createMapReducerOSMContribution()
            .timestamps(timestamps72)
            .aggregateByTimestamp()
            .sum(contribution -> 1);
    SortedMap<OSHDBTimestamp, Integer> resultCustom =
        createMapReducerOSMContribution()
            .timestamps(timestamps72)
    ...
    for (OSHDBTimestamp t : resultAuto.keySet()) {
      assertEquals(resultAuto.get(t).intValue(), resultCustom.get(t).intValue());
    }
  }
  ...
  public void testOSMEntitySnapshot() throws Exception {
    ...
    // multiple timestamps
    SortedMap<OSHDBTimestamp, Integer> result72 =
        createMapReducerOSMEntitySnapshot()
            .timestamps(timestamps72)
            .aggregateByTimestamp()
            .sum(snapshot -> 1);
  ...
  public void testMapperFunctions() throws Exception {
    ...
    for (OSHDBTimestamp t : result1.keySet()) {
      assertEquals(result1.get(t).intValue(), result2.get(t).intValue());
    }
  }
  ...
  public void testCombinedWithAggregateByIndex() throws Exception {
    ...
    SortedMap<OSHDBTimestamp, SortedMap<OSMType, Integer>> nestedResult1 =
