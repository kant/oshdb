...
public abstract class OSHEntity2 {
  ...
  protected abstract static class OSHBuilder {
    ...
    public void build(
        ...
        List<? extends OSMEntity> versions,
        long baseTimestamp,
        long baseLongitude,
        long baseLatitude,
        Map<Long, Integer> nodeOffsets,
        ...
        if (visible == 1) {
          ...
          if (!Arrays.equals(version.getRawTags(), tags)) {
            ...
            for (int i = 0; i < tags.length; i++) {
              ...
              if (i % 2 == 0) keySet.add(Integer.valueOf(tags[i]));
            }
          }
          if (extension(
              aux,
    ...
    protected abstract boolean extension(
        ByteArrayOutputWrapper out,
        OSMEntity version,
        long baseLongitude,
        long baseLatitude,
