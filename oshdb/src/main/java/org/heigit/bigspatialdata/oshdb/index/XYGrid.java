...
/**
 * XYGrid spans an equal degree grid over the world. <br>
 * <br>
 * IDs for zoom = 2:
 *
...
public class XYGrid implements Serializable {
  ...
  public long getEstimatedIdCount(final OSHDBBoundingBox data) {
    ...
    return ((long)
        Math.ceil(
            Math.max(
                (data.getMaxLonLong() - data.getMinLonLong()) / cellWidth,
                (data.getMaxLatLong() - data.getMinLatLong()) / cellWidth)));
  }
  ...
  /**
   * Calculates all tiles, that lie within a bounding-box. TODO but priority 999: Add possibility to
   * snap the BBX to the tile-grid. TODO: is an exception needed?
   *
   * @param bbox The bounding box. First dimension is longitude, second is latitude.
   * @param enlarge if true, the BBOX is enlarged by one tile to the south-west to include tiles
   *     that possibly hold way or relation information, if false only holds tiles that intersect
   *     with the given BBOX. For queries: false is for nodes while true is for ways and relations.
  ...
  public Set<Pair<Long, Long>> bbox2CellIdRanges(OSHDBBoundingBox bbox, boolean enlarge) {
    ...
    if (minlat > maxlat) {
      LOG.warn(
          "The minimum values are not smaller than the maximum values. This might throw an exeption one day?");
      return null;
    }
      ...
      result.addAll(
          bbox2CellIdRanges(
              new OSHDBBoundingBox(
                  minlong, minlat, (long) (180.0 * OSHDB.GEOM_PRECISION_TO_LONG) - 1L, maxlat),
              enlarge));
    ...
    if (enlarge) {
      // it is impossible vor features to span over the datelimit, so the enlargement stops at
      // column 0
      if (columnmin > 0) {
        columnmin -= 1;
      }
